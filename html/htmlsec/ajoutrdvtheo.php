...
$sql2="select * from salle where num_salle='$salle' ";
$result2=mysqli_query($con,$sql2);

$sql3="select * from salle where num_salle='$salle' and capacité in (select nb_part from rdv_theo where salle='$salle' and num_seance='$nums')";
$result3=mysqli_query($con,$sql3);

$sql4="select cin3 from rdv_pratique where cin3='$idm' and date='$date' and heure='$heure'";
$result4=mysqli_query($con,$sql4);

$sql5="select id_moniteur from rdv_theo where id_moniteur='$idm' and date='$date' and heure='$heure' and num_seance!='$nums' ";
...
$sql8="select salle from rdv_theo where salle='$salle' and date='$date' and heure='$heure' and num_seance!='$nums' ";
...
elseif (mysqli_num_rows($result2)!=1){
    echo '<script type="text/javascript"> '; 
    echo 'alert("Salle inexistante") ; ';
    echo' </script>';
}
elseif (mysqli_num_rows($result3)==1){
    echo '<script type="text/javascript"> ';
    echo 'alert("capacité de salle saturée !") ; ';
    echo' </script>';
}
...
elseif (mysqli_num_rows($result8)!=0){
    echo '<script type="text/javascript"> ';
    echo 'alert("Salle utilisée pour un autre rendez-vous!") ; ';
    echo' </script>';
}
...
insert into rdv_theo (id_rdv,num_seance,date,heure,id_moniteur,salle,id_candidat)  
values ('','$nums','$date','$heure','$idm','$salle','$idc');
...
<head>
    ...
    <title>Ajout Rendez-vous théorique</title>
</head>
<body>
...
<nav  class="navbar navbar-expand-lg navbar-light " style="background-color: #8a888285;" >
    ...
    <fieldset>
        <form method="POST" >  
            <label>Numéro séance :</label>
            <input type="name" name="nums" /><br>
